/*  =========================================================================
    rfc_hello_selftest.c - run selftests
...
// Tests for stable public classes:
    ...
    { "baltree", baltree_test, true, true, NULL },
    { "hello", hello_test, true, true, NULL },
...
{
    test_item_t *item;
    printf ("Running rfc-hello selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef RFC_HELLO_BUILD_DRAFT_API // selftest is still in draft
        else
            rfc_hello_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("rfc_hello_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running rfc-hello test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef RFC_HELLO_BUILD_DRAFT_API // selftest is still in draft
        else
            rfc_hello_private_selftest (verbose, test->subtest);
#endif // RFC_HELLO_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
